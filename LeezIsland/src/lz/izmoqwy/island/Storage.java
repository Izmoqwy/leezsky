...
public class Storage {
	...
	static {
		...
		try {
			...
			DB.execute("CREATE TABLE IF NOT EXISTS `Islands` ( `island_id` VARCHAR(12) UNIQUE, `leader` VARCHAR(36) UNIQUE, `name` VARCHAR(36) UNIQUE, `level` INTEGER(7), " +
					"`settings` VARCHAR(128), `general` VARCHAR(255), `members` TEXT )");
