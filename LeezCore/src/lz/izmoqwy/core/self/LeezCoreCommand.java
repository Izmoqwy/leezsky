...
public class LeezCoreCommand extends CoreCommand {
	...
	public LeezCoreCommand() {
		super("leezcore", CommandOptions.builder()
				.permission("leezsky.core.command").needsArg(true)
				.build());
	}
		...
		switch (args[0].toLowerCase()) {
			case "backup":
				...
				if (file.exists()) {
					...
					commandSender.sendMessage(LeezCore.PREFIX + "§aBackup restorée. Nous sommes désolé pour la gêne occasionnée.");
				}
				else {
			...
			case "reload":
			case "rl":
				checkPermission(commandSender, "admin.reload");
